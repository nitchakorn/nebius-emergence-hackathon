        ...
        class Handler(BaseHTTPRequestHandler):
            def do_GET(self):
                ...
                if parsed.path != "/callback":
                    # Ignore stray local requests (e.g. a browser's automatic
                    # favicon.ico fetch, or a second concurrent login attempt hitting
                    # this same port). Only the real redirect satisfies the wait below
                    # — otherwise a stray request can be captured instead of the real
                    # one, producing a bogus/missing OAuth state.
