"""CLI entrypoint and orchestration for ClinicalxCRAFT.

The investigation is LLM-orchestrated: the model discovers the schema, forms and tests
hypotheses, and writes the report via the loop in llm.run_investigation, backed by a
ToolExecutor over the live CRAFT MCP session. All logic lives in run(); the CLI and the
Streamlit UI both call it.
"""
import argparse
import asyncio
import json
...
# A verified investigation prompt that exercises the clinical, mutation, and protein
# tables — used as the CLI default and as an example in the UI.
DEFAULT_QUESTION = (
    "In this KIRC (kidney cancer) cohort, do patients with a VHL mutation differ in "
    "outcome or PI3K/AKT/mTOR protein signaling from those without one?"
)
...
async def run(
    ...
    async with CraftClient() as client:
        ...
        report = await run_investigation(executor, question, on_event=on_event, context=context)
...
def main():
    parser = argparse.ArgumentParser(description="ClinicalxCRAFT — clinical/genomic cohort investigator")
